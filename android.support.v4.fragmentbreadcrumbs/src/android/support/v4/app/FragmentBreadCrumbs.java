...
import android.support.v4.fragmentbreadcrumbs.R;
 ...
 * stack in an activity. This is intended to be used with {@link ActionBar#setCustomView(View)
		...
		implements FragmentManager.OnBackStackChangedListener
{
	...
	private static final int DEFAULT_GRAVITY = GravityCompat.START | Gravity.CENTER_VERTICAL;
	...
	public FragmentBreadCrumbs(Context context, AttributeSet attrs, int defStyleAttr)
	{
		...
		final TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.FragmentBreadCrumbs, defStyleAttr, 0);

		mGravity = a.getInt(R.styleable.FragmentBreadCrumbs_gravity,
				DEFAULT_GRAVITY);
		mLayoutResId = a.getResourceId(
				R.styleable.FragmentBreadCrumbs_itemLayout,
				R.layout.fragment_bread_crumb_item);
		mTextColor = a.getColor(
				R.styleable.FragmentBreadCrumbs_itemColor,
				0);
	 ...
	 * entry will result in a call to the specified listener's {@link android.view.View.OnClickListener#onClick(View)} method.
	 *
	 * @param title
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b)
	{
		...
		final int childBottom = getPaddingTop() + child.getMeasuredHeight() - getPaddingBottom();

		int childLeft;
		int childRight;

		final int layoutDirection = ViewCompat.getLayoutDirection(this);
		final int horizontalGravity = mGravity & GravityCompat.RELATIVE_HORIZONTAL_GRAVITY_MASK;
		switch (GravityCompat.getAbsoluteGravity(horizontalGravity, layoutDirection))
		{
			case Gravity.RIGHT:
				childRight = getRight() - getLeft() - getPaddingRight();
				childLeft = childRight - child.getMeasuredWidth();
				break;

			case Gravity.CENTER_HORIZONTAL:
				childLeft = getPaddingLeft() + (getRight() - getLeft() - child.getMeasuredWidth()) / 2;
		...
		if (childRight > getRight() - getLeft() - getPaddingRight())
		{
			childRight = getRight() - getLeft() - getPaddingRight();
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (child.getVisibility() != GONE)
			{
				...
				measuredChildState = ViewCompat.combineMeasuredStates(measuredChildState,
